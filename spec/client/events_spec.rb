...
RSpec.describe MobilizeAmericaClient::Client::Events do
  ...
  describe '#organization_events' do
    ...
    it 'should raise if response status is 404' do
      stub_request(:get, events_url).with(headers: standard_headers).to_return(status: 404, body: {error: 'not found'}.to_json, headers: response_headers)
    ...
    it 'should call the endpoint and return JSON' do
      stub_request(:get, events_url).with(headers: standard_headers).to_return(body: response.to_json, headers: response_headers)
    ...
    it 'should escape the organization ID' do
      ...
      stub_request(:get, expected_url).with(headers: standard_headers).to_return(body: response.to_json, headers: response_headers)
